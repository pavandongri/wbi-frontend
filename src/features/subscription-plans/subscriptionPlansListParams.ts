import type { SubscriptionPlansSortBy } from "@/types/subscription-plans.types";

export type SubscriptionPlansListUrlState = {
  page: number;
  limit: number;
  q: string;
...
const SORTABLE = new Set<SubscriptionPlansSortBy>([
  "name",
  "code",
  "amount",
  "interval",
...
export function parseSubscriptionPlansListParams(
  sp: Pick<URLSearchParams, "get">
): SubscriptionPlansListUrlState {
  ...
  const sortBy =
    sortCandidate && SORTABLE.has(sortCandidate as SubscriptionPlansSortBy)
      ? (sortCandidate as SubscriptionPlansSortBy)
      : "createdAt";
  const sortOrder = sp.get("sortOrder") === "asc" ? "asc" : "desc";
  return { page, limit, q, sortBy, sortOrder };
}

export function stringifySubscriptionPlansListParams(state: SubscriptionPlansListUrlState): string {
