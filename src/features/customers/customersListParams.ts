import type { CustomersSortBy } from "@/types/customers.types";

export type CustomersListUrlState = {
  page: number;
  limit: number;
  q: string;
...
const SORTABLE = new Set<CustomersSortBy>([
  "name",
  "phone",
  "email",
  "city",
...
export function parseCustomersListParams(sp: Pick<URLSearchParams, "get">): CustomersListUrlState {
  ...
  const sortBy =
    sortCandidate && SORTABLE.has(sortCandidate as CustomersSortBy)
      ? (sortCandidate as CustomersSortBy)
      : "createdAt";
  const sortOrder = sp.get("sortOrder") === "asc" ? "asc" : "desc";
  return { page, limit, q, sortBy, sortOrder };
}

export function stringifyCustomersListParams(state: CustomersListUrlState): string {
